...
def show_buttons_photo_need_yes_no(message: Message) -> None:
    ...
    """
    logger.info(
        f"Вывод кнопок о необходимости фотографий пользователю. User_id: {message.chat.id}"
    )
...
def show_cities_buttons(message: Message, possible_cities: Dict) -> None:
    ...
    """
    logger.info(
        f"Вывод кнопок с вариантами городов пользователю. User_id: {message.chat.id}"
    )
