...
import handlers.custom.input_data
...
def input_date(call: CallbackQuery) -> None:
    ...
    if action == "DAY":
        logger.info(
            f"Выбрана какая-то дата, нужно ее проверить. User_id: {call.message.chat.id}"
        )
        ...
        with bot.retrieve_data(call.message.chat.id) as data:
            if "checkInDate" in data:
                ...
                if int(select_date) > checkin:
                    logger.info(
                        f"Ввод и сохранение даты выезда. User_id: {call.message.chat.id}"
                    )
                    ...
                    if data["sort"] == "DISTANCE":
                        ...
                        utility.show_data_and_find_hotels.print_data(call.message, data)
                else:
                    bot.send_message(
                        call.message.chat.id,
                    ...
                    handlers.custom.input_data.my_calendar(call.message, "выезда")
            else:
                logger.info(
                    f"Ввод и сохранение даты заезда. User_id: {call.message.chat.id}"
                )
