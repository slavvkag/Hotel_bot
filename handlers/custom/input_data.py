...
def low_high_best_handler(message: Message) -> None:
    ...
    bot.send_message(
        ...
        "Введите город в котором нужно найти отель (на латинице): ",
    )
...
def input_city(message: Message) -> None:
    ...
    with bot.retrieve_data(message.chat.id) as data:
        ...
        logger.info(
            "Пользователь ввел город: " + message.text + f" User_id: {message.chat.id}"
        )
        ...
        if response_cities.status_code == 200:
            logger.info(
                "Сервер ответил: "
                + str(response_cities.status_code)
                + f" User_id: {message.chat.id}"
            )
            possible_cities = utility.processing_json.get_city(response_cities.text)
            keyboards.inline.create_buttons.show_cities_buttons(
            ...
            bot.send_message(
                message.chat.id, "Нажмите команду еще раз. И введите другой город."
            )
            data.clear()
...
def input_quantity(message: Message) -> None:
    ...
    if message.text.isdigit():
        if 0 < int(message.text) <= 25:
            logger.info(
                "Ввод и запись количества отелей: "
                + message.text
                + f" User_id: {message.chat.id}"
            )
            with bot.retrieve_data(message.chat.id) as data:
            ...
            bot.send_message(
                message.chat.id, "Введите минимальную стоимость отеля в долларах США:"
            )
        else:
            bot.send_message(
                message.chat.id,
                "Ошибка! Это должно быть число в диапазоне от 1 до 25! Повторите ввод!",
            )
    else:
...
def input_price_min(message: Message) -> None:
    ...
    if message.text.isdigit():
        logger.info(
            "Ввод и запись минимальной стоимости отеля: "
            + message.text
            + f" User_id: {message.chat.id}"
        )
        with bot.retrieve_data(message.chat.id) as data:
        ...
        bot.send_message(
            message.chat.id, "Введите максимальную стоимость отеля в долларах США:"
        )
    else:
        bot.send_message(message.chat.id, "Ошибка! Вы ввели не число! Повторите ввод!")
...
def input_photo_quantity(message: Message) -> None:
    ...
    if message.text.isdigit():
        if 0 < int(message.text) <= 10:
            logger.info(
                "Ввод и запись количества фотографий: "
                + message.text
                + f" User_id: {message.chat.id}"
            )
            with bot.retrieve_data(message.chat.id) as data:
...
def input_landmark_in(message: Message) -> None:
    ...
    if message.text.isdigit():
        logger.info(
            "Ввод и запись начала диапазона от центра: "
            + message.text
            + f" User_id: {message.chat.id}"
        )
        with bot.retrieve_data(message.chat.id) as data:
        ...
        bot.send_message(
            message.chat.id, "Введите конец диапазона расстояния от центра (в милях)."
        )
    else:
        bot.send_message(message.chat.id, "Ошибка! Вы ввели не число! Повторите ввод!")
...
def input_landmark_out(message: Message) -> None:
    ...
    if message.text.isdigit():
        logger.info(
            "Ввод и запись конца диапазона от центра: "
            + message.text
            + f" User_id: {message.chat.id}"
        )
        with bot.retrieve_data(message.chat.id) as data:
