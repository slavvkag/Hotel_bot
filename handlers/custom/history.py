...
def get_list_history(message: Message) -> None:
    ...
    if queries:
        logger.info(
            f"Получены записи из таблицы query:\n {queries}. User_id: {message.chat.id}"
        )
        for item in queries:
            bot.send_message(
                message.chat.id,
                f"({item[0]}). Дата и время: {item[1]}. Вы вводили город: {item[2]}",
            )
        bot.set_state(message.chat.id, UserInputState.select_number)
        bot.send_message(
            message.from_user.id, "Введите номер интересующего вас варианта: "
        )
    else:
    ...
    if message.text.isdigit():
        ...
        if int(message.text) in number_query:
            ...
            if history_dict:
                ...
                bot.send_message(
                    message.chat.id, "Почему-то ответ пуст! Попробуйте другие операции."
                )
