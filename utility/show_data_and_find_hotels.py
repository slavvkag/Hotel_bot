...
def find_and_show_hotels(message: Message, data: Dict) -> None:
    ...
    logger.info(
        f"Сервер вернул ответ {response_hotels.status_code}. User_id: {message.chat.id}"
    )
    ...
    if response_hotels.status_code == 200:
        ...
        for hotel in hotels.values():
            ...
            if count < int(data["quantity_hotels"]):
                ...
                get_summary = utility.api_request.request(
                    "POST", summary_url, summary_payload
                )
                logger.info(
                    f"Сервер вернул ответ {get_summary.status_code}. User_id: {message.chat.id}"
                )
                if get_summary.status_code == 200:
                    ...
                    if int(data["photo_count"]) > 0:
                        ...
                        logger.info(
                            f"Выдаю найденную информацию в чат. User_id: {message.chat.id}"
                        )
...
def print_data(message: Message, data: Dict) -> None:
    ...
    logger.info(
        f"Вывод суммарной информации о параметрах запроса пользователем. User_id: {message.chat.id}"
    )
    text_message = (
        "Исходные данные:\n"
